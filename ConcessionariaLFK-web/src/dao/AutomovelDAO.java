...
public class AutomovelDAO {
	...
	public Automovel inserir(Automovel automovel) throws Exception {
		String sqlInsert =
			"INSERT INTO Automovel (modelo, fabricante, ano, cilindrada, preco,  tipo) VALUES (?, ?, ?, ?, ?, ?)";
		...
		try {
			...
			ps.setFloat(5, automovel.getPreco());
	...
	public Automovel atualizar(Automovel automovel) throws Exception {
		String sqlInsert =
			"UPDATE Automovel SET modelo = ?, fabricante = ?, ano = ?, cilindrada = ?,preco = ?, tipo = ? where codigo = ?";
		...
		try {
			...
			ps.setFloat(5, automovel.getPreco());
			ps.setInt(6, automovel.getTipo());
			ps.setInt(7, automovel.getCodigo());
			int ret = ps.executeUpdate();
			if (ret != 1) {
